...
class Schedule{
    ...
    Dictionary<int, int[]> tram15_schedule = new Dictionary<int, int[]>();
        ...
        foreach(string text in list_of_txt){
            ...
            StreamReader path = new StreamReader(text);
            string prev_split = path.ReadLine()!;

            while(prev_split != null){
                string[] split = prev_split.Split(' ', StringSplitOptions.RemoveEmptyEntries);
                if(split.Length != 1){
                    int[] temp_split = split[1 .. ^1].Select(int.Parse).ToArray();
                    list_of_schedule[count][int.Parse(split[0])] = temp_split;
                    }
                else{
                    int[] empty = new int[0];
                    list_of_schedule[count][int.Parse(split[0])] = empty;
                    }
                prev_split=path.ReadLine()!;
    ...
    class Graph{
        ...
        Dictionary<string, string> graph_state = new Dictionary<string, string>();
        ...
        void makeGraph(){
            ...
            StreamReader path = new StreamReader("graph_adj.txt");
            string str = path.ReadLine()!;
            str = path.ReadLine()!;
        ...
        class Time{
            ...
            (int, int) processTime(int weight){ //hour // mins
            ...
            class Heap{
                ...
                void downHeap(int i){
                    ...
                    if(this.length > 0){
                        ...
                        (string smallest_node, int smallest_val) = this.values![smallest];
                        (string node, int node_val) = this.values![left_child];

                        if(left_child < this.length && node_val > smallest_val && node != null){
                            smallest = left_child;
                        }

                        (node, node_val) = this.values![right_child];

                        if(right_child < this.length && node_val > smallest_val && node != null){
                            smallest = right_child;
                        }
                        if(smallest != i){
                            (string, int) temp = this.values![i];
                ...
                (bool, int) searchNode((string, int) node){
                    ...
                    while(lo <= hi){
                        int mid = (lo+hi)/2;
                        if(this.values![mid].Item1 == node.Item1){
                            return (true, mid);
                        }
                        if(node.Item2 < mid){
                            hi = mid -1;
                ...
                void decreaseKey((string, int) node){

                    (bool contain, int index) = searchNode(node);
                    if(!contain){
                        addValue(node);
                        return;
                    }
                ...
                void sort(){ //remove_smallest debugg
                    int temp_length = this.length;
                    while(this.length > 0){
                        (string, int) x;
                        x = this.removeSmallest();
                ...
                public static void printRoot(List<string> root, Graph graph, Time time_manage){
                    ...
                    int total_weigth = graph.h["Mala_Strana"] - graph.h["Listopadu"];
                    ...
                    for(int i = root.Count-1; i >= 0; i--){
                        if(i == root.Count-1){ 
                            ...
                            //WriteLine(graph.weight[("Listopadu", root[i])] - graph.temp_weight[("Listopadu", root[i])]);
                            //int hour = leaving_time.Item1;
                            //int minute = leaving_time.Item2;
                ...
                public static void makeRoot(string node, Graph graph, List<string> root, Time time_manage){
                    if(graph.prev_node[node] == "undifined"){
                        printRoot(root, graph, time_manage);
                    }
                    else{
                        root.Add(node);
                        //WriteLine($"{node} weight is {graph.h[node]}");
                        makeRoot(graph.prev_node[node], graph, root, time_manage);
                    }   
                }
                public void heapsort((string, int)[] list_of_nodes){
                    Heap h = new Heap();
                    h.minHeap(list_of_nodes);
                    h.sort();
                }
                ///
                ...
                public static int findNiceSchedule(Dictionary<int, int[]> way, Time time_manage, int weight, string node){ //memo: the shcedule is not matched with the result. guess timer is somehow wrong
                    ...
                    //WriteLine($"{curr_weighed_time.Item1+1} curr_weighed_time.Item1, {node}");
                    if(way[hour+1].Length == 0){
                ...
                public static void dijkstra(string start, string goal, Graph graph, Schedule schedule, Time time_manage){ //probbably there is error in the metro stuff.
                    ...
                    foreach(string node in graph.graph.Keys){
                        ...
                        foreach(string neighbor in graph.graph[min_next_node]){
                            ...
                            //WriteLine($"New weight: {new_weight}, temp edge:{temp_edge}");
                            ...
                            if (temp < graph.h[neighbor]){       
                                ...
                                //heap.add_value((neighbor, temp));
                            }
                        }
                    graph.graph_state[node] = "closed";
                    }
